// internal/controllers/http/rest/handler.go
package rest
...
func (h *Handler) CreateUser(w http.ResponseWriter, r *http.Request) {
	...
	// The password from the request is temporarily placed in PasswordHash.
	// The use case layer will hash it before saving.
	user := &entity.User{
		NickName:           req.Name,
		Email:              &req.Email,
...
func (h *Handler) Logout(w http.ResponseWriter, r *http.Request) {
	// JWTトークンの場合、クライアント側でトークンを削除
	// サーバー側でトークンブラックリストを管理する場合は、ここで処理
...
func (h *Handler) CreateMessage(w http.ResponseWriter, r *http.Request) {
	...
	// この例では、リクエストボディにUserIDが含まれていると仮定する
	// senderID := r.Context().Value("userID").(string)
	senderID := "some-user-id" // 仮のID
...
func (h *Handler) UpdateMessage(w http.ResponseWriter, r *http.Request) {
	...
	// TODO: 認証情報とSenderIDを比較し、更新権限を確認する
	// if message.SenderID != r.Context().Value("userID").(string) {
	// 	h.sendErrorResponse(w, http.StatusForbidden, "You are not allowed to update this message")
	// 	return
	// }
...
func (h *Handler) DeleteMessage(w http.ResponseWriter, r *http.Request) {
	...
	// TODO: 認証情報とSenderIDを比較し、削除権限を確認する
	// message, err := h.messageUsecase.GetMessageByID(messageID)
	// if err != nil {
	// 	h.sendErrorResponse(w, http.StatusNotFound, "Message not found")
	// 	return
	// }
	// if message.SenderID != r.Context().Value("userID").(string) {
	// 	h.sendErrorResponse(w, http.StatusForbidden, "You are not allowed to delete this message")
	// 	return
	// }
